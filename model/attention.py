...
class ProbAttention(nn.Module):
    ...
    def forward(self, queries, keys, values, attention_mask):
        ...
        context, attention = self._update_context(context, values, scores_top, index, L, attention_mask)
...
class AttentionLayer(nn.Module):
    def __init__(self, attention, d_model, n_heads, d_keys=None, d_values=None):
        ...
        self.values_projection = nn.Linear(d_model, d_values * n_heads)
    ...
    def forward(self, queries, keys, values, attention_mask):
        ...
        values = self.values_projection(values).view(B, S, H, -1)
